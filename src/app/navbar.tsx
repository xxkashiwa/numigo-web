    ...
    <div className="sticky flex w-full items-center justify-between border-b border-primary/20 bg-background">
      ...
      <div className="inline-flex">
        {isAuthenticated ? (
          ...
          <button className="navi-item" onClick={() => router.push('/login')}>
            <NoAccountIcon />
          </button>
        )}
      </div>
...
const ModelSelector = () => {
  ...
  return (
    <Select
      ...
      value={currentModel}
    >
      <SelectTrigger className="navi-item w-fit focus:ring-0">
        <div className="flex items-center">
          <SelectValue placeholder="Select Model" />
        </div>
      </SelectTrigger>
      <SelectContent className="navi-item backdrop-blur-sm">
        <SelectGroup>
